...
import 'package:code_initial/domain/models/panier_page_model.dart';
...
class CheckoutController extends GetxController {
  ...
  List<PanierPageModel> get cartItems => Get.find<PanierController>().panierpage;
  ...
  void updateShippingAddress({
    ...
    shippingAddress.update((val) {
      val!.fullName = fullName ?? val.fullName;
      val.addressLine1 = addressLine1 ?? val.addressLine1;
      if (addressLine2 != null) val.addressLine2 = addressLine2;
      val.city = city ?? val.city;
      val.state = state ?? val.state;
      val.postalCode = postalCode ?? val.postalCode;
      val.country = country ?? val.country;
      val.phoneNumber = phoneNumber ?? val.phoneNumber;
  ...
  Future<void> placeOrder() async {
    if (_validateForm()) {
      try {
        ...
        orderId = 'CMD${DateTime.now().millisecondsSinceEpoch.toString().substring(5)}';
  ...
  OrderModel _createOrder() {
    ...
    final orderItems = cartItems.map((item) => OrderItem(
      productId: item.id ?? '${item.hashCode}',
      name: item.nom,
